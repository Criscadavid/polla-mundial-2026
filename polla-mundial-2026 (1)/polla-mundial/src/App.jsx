...
function groupMatchList(g) {
  return [
    ...JORNADAS.j1.map(p => ({ pair:p, jornada:1 })),
    ...JORNADAS.j2.map(p => ({ pair:p, jornada:2 })),
    ...JORNADAS.j3.map(p => ({ pair:p, jornada:3 })),
  ];
}
...
const PLBL = {
  setup:   '⚙ Configuración',
  p1_open: '📝 Predicciones abiertas',
  groups:  '⚽ Fase de grupos en curso',
  done:    '✅ Tiempo 1 finalizado',
};

// ── TABLA AUTOMÁTICA ──────────────────────────────────────────
function calcGroupStandings(g, gm) {
  const teams = GROUPS[g];
  const pts=[0,0,0,0], gf=[0,0,0,0], gc=[0,0,0,0];
...
function calcAllStandings(gm) {
  const s = {};
  GK.forEach(g => { s[g] = calcGroupStandings(g, gm); });
  return s;
}

function calcBest8Thirds(gm) {
  return GK.map(g => {
    const s = calcGroupStandings(g, gm);
...
function calcTotal(t1, rr) {
  if (!t1) return { total:0, exactAll:0 };
  let total=0, exactAll=0;
  GK.forEach(g => {
    groupMatchList(g).forEach(({ pair:[i,j] }) => {
...
const S = {
  ...
  async list(pfx) {
    try { const r = await window.storage.list(pfx); return r?.keys || []; }
    catch { return []; }
  },
};
...
function exportToCSV(participants, rr) {
  ...
  participants.filter(p => p.t1?.submitted).forEach(p => {
    ...
    detail.push(['Grupo','Partido','Local','Pred L','Pred V','Visitante','Real','Pts']);
    GK.forEach(g => {
      const teams = GROUPS[g];
      groupMatchList(g).forEach(({ pair:[i,j] }) => {
        const k = matchKey(g,i,j);
        ...
        detail.push([`Gr.${g}`, `J${groupMatchList(g).findIndex(m=>matchKey(g,...m.pair)===k)<2?1:groupMatchList(g).findIndex(m=>matchKey(g,...m.pair)===k)<4?2:3}`, teams[i], pred?.h??'', pred?.a??'', teams[j], realRes, pts]);
      });
    });
    detail.push(['','','','','','','TOTAL:', calcTotal(p.t1,rr).total]);
    detail.push(['R32:',...(p.t1?.r32teams||[])]);
  });
  const csv = detail.map(r => r.map(c => `"${String(c).replace(/"/g,'""')}"`).join(',')).join('\n');
  const blob = new Blob(['\uFEFF'+csv], { type:'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = `polla-backup-${new Date().toISOString().slice(0,10)}.csv`;
  a.click(); URL.revokeObjectURL(url);
}

// ── THEME ─────────────────────────────────────────────────────
const C = {
  bg:'#070f06', card:'#0d1a0b', card2:'#101f0e', brd:'#1c3419',
  gold:'#f7c948', grn:'#22c55e', txt:'#f0f4ef', muted:'#607a5e', red:'#ef4444',
};

// ── PRIMITIVES ────────────────────────────────────────────────
function Btn({ label, onClick, v='gold', dis, full=true, sm, sx }) {
  const M = {
    gold:  { bg:C.gold, col:'#070f06', br:'none' },
    green: { bg:C.grn,  col:'#070f06', br:'none' },
    dark:  { bg:C.card, col:C.muted,   br:`1px solid ${C.brd}` },
    red:   { bg:C.red,  col:'#fff',    br:'none' },
  };
  const m = M[v] || M.gold;
  return (
    <button onClick={onClick} disabled={dis} style={{
      background:m.bg, color:m.col, border:m.br, borderRadius:8,
      padding: sm?'6px 14px':'10px 20px', fontSize:sm?12:14, fontWeight:700,
      cursor:dis?'not-allowed':'pointer', opacity:dis?.45:1,
      fontFamily:"'Nunito',sans-serif", width:full?'100%':'auto', ...sx,
    }}>{label}</button>
  );
}

function Card({ children, sx, hl }) {
  return (
    <div style={{
      background:C.card, border:`1px solid ${hl?C.gold:C.brd}`,
      borderRadius:12, padding:16, ...sx,
    }}>{children}</div>
  );
}

function Inp({ val, set, ph, type='text', sx }) {
  return (
    <input value={val} onChange={e=>set(e.target.value)} placeholder={ph} type={type} style={{
      ...
      padding:'8px 12px', color:C.txt, fontSize:14, outline:'none',
      fontFamily:"'Nunito',sans-serif", width:'100%', ...sx,
    }} />
  );
}
...
function ScoreBox({ val={}, set, dis }) {
  const si = {
    width:42, textAlign:'center', background:C.bg, border:`1px solid ${C.brd}`,
    borderRadius:6, padding:'6px 0', color:C.txt, fontSize:16, fontWeight:700,
    outline:'none', fontFamily:"'Nunito',sans-serif",
  };
  return (
    <div style={{ display:'flex', alignItems:'center', gap:6 }}>
      <input type="number" min="0" max="30" value={val.h??''} disabled={dis} style={si}
        onChange={e=>set({...val,h:e.target.value})} />
      <span style={{ color:C.muted, fontSize:13, fontWeight:700 }}>:</span>
      <input type="number" min="0" max="30" value={val.a??''} disabled={dis} style={si}
        onChange={e=>set({...val,a:e.target.value})} />
    </div>
  );
}
...
function Back({ go }) {
  return (
    <button onClick={go} style={{
      ...
      padding:'4px 0', fontSize:13, fontFamily:"'Nunito',sans-serif",
      display:'flex', alignItems:'center', gap:4, marginBottom:10,
    }}>← Volver</button>
  );
}

function Title({ text, sz=32, sx }) {
  return <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:sz, color:C.gold, lineHeight:1, ...sx }}>{text}</div>;
}

function Lbl({ text }) {
  return <div style={{ fontSize:11, color:C.muted, textTransform:'uppercase', letterSpacing:.6, marginBottom:5 }}>{text}</div>;
}
    ...
    <div style={{ display:'flex', gap:4, overflowX:'auto', marginBottom:14, paddingBottom:2 }}>
      {tabs.map(t => (
        <button key={t.id} onClick={()=>set(t.id)} style={{
          background:t.id===active?C.gold:C.card, color:t.id===active?'#070f06':C.muted,
          border:'none', borderRadius:7, padding:'7px 12px', fontSize:11, fontWeight:700,
...
function Home({ cfg, nav }) {
  return (
    <div style={{ maxWidth:420, margin:'0 auto', padding:'60px 24px', textAlign:'center' }}>
      ...
      <div style={{ display:'inline-block', background:C.card, border:`1px solid ${C.brd}`, borderRadius:20, padding:'4px 14px', fontSize:12, color:C.muted, marginBottom:40 }}>
        {PLBL[cfg?.phase||'setup']}
      </div>
...
function Admin({ cfg, saveCfg, back }) {
  ...
  useEffect(() => {
    if (auth) S.get('rr').then(r => setRr(r || { gm:{}, cl:{} }));
  }, [auth]);
  ...
  if (!cfg?.adminPwd || !auth) {
    ...
    return (
      <div style={{ maxWidth:360, margin:'0 auto', padding:'40px 20px' }}>
        <Back go={back} />
        <Title text={isNew ? 'CREAR ADMIN' : 'PANEL ADMIN'} />
  ...
  const TABS = [
    ...
    { id:'gr',     label:'⚽ Resultados' },
    { id:'cl',     label:'✅ Clasificados' },
    { id:'pl',     label:'👥 Participantes' },
  ];
...
function AStatus({ cfg, saveCfg }) {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
      {cfg.phase==='groups' && (
        <div style={{ background:'#1a1500', border:`1px solid ${C.gold}`, borderRadius:10, padding:'12px 14px', display:'flex', gap:10, alignItems:'flex-start' }}>
          ...
          <div>
            ...
            <div style={{ color:C.muted, fontSize:12, lineHeight:1.5 }}>
              Cuando terminen los 72 partidos, ve a <span style={{ color:C.txt, fontWeight:700 }}>✅ Clasificados</span> e ingresa y guarda los 32 clasificados. Sin ese paso el scoring de clasificados no funcionará.
            </div>
          </div>
        </div>
      )}
      <Card>
        ...
        {pi < PHASES.length-1 && (
          <Btn v="green" label={`➡ Avanzar a: ${PLBL[PHASES[pi+1]]}`}
            onClick={() => saveCfg({ ...cfg, phase:PHASES[pi+1] })} />
        )}
        {pi > 0 && (
          <Btn v="dark" label="⬅ Retroceder" sx={{ marginTop:8 }}
            onClick={() => saveCfg({ ...cfg, phase:PHASES[pi-1] })} />
        )}
      </Card>
      <Card>
        ...
        <div style={{ color:C.muted, fontSize:12, marginBottom:10 }}>
          Las predicciones se bloquean automáticamente en esta fecha. El admin puede extenderla retrocediendo la fase.
        </div>
        <input type="datetime-local" value={closingDate}
          onChange={e => { setClosingDate(e.target.value); setSaved(false); }}
        ...
        <Btn label={saved?'✓ Guardado':'Guardar fecha de cierre'} v="green"
          onClick={async () => { await saveCfg({ ...cfg, closingDate }); setSaved(true); }} />
        {cfg.closingDate && (
          <div style={{ fontSize:11, color:C.muted, marginTop:8 }}>
            Cierre actual: {new Date(cfg.closingDate).toLocaleString('es-CO', { timeZone:'America/Bogota' })}
          </div>
        )}
      </Card>
      <Card>
        {PHASES.map((p,i) => (
          <div key={p} style={{ display:'flex', alignItems:'center', gap:10, padding:'7px 0', borderBottom:i<PHASES.length-1?`1px solid ${C.brd}`:'none' }}>
            <span style={{ fontSize:14, color:i<pi?C.grn:i===pi?C.gold:'#ffffff22', width:16 }}>
              {i<pi?'✓':i===pi?'▶':'○'}
            </span>
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:4, flexWrap:'wrap', marginBottom:12 }}>
        {GK.map(g => (
          <button key={g} onClick={()=>setAg(g)} style={{
            background:g===ag?C.gold:C.card, color:g===ag?'#070f06':C.muted,
            border:'none', borderRadius:6, padding:'5px 11px', fontSize:12,
        ...
        {[1,2,3].map(j => (
          <div key={j}>
            ...
            {matches.filter(m=>m.jornada===j).map(({ pair:[i,k] }) => {
              ...
              return (
                <div key={mk} style={{ display:'flex', alignItems:'center', padding:'8px 0', borderBottom:`1px solid ${C.brd}` }}>
      ...
      <Card sx={{ marginBottom:10 }}>
        ...
        {standings.map((s,i) => (
          <div key={s.team} style={{ display:'flex', alignItems:'center', gap:8, padding:'5px 0', borderBottom:i<3?`1px solid ${C.brd}`:'none' }}>
      ...
      <Btn label={ok?'✓ Guardado':'Guardar resultados'} v="green"
        onClick={async () => { await save({ ...rr, gm:loc }); setOk(true); }} />
    </div>
  );
}
...
function ACL({ rr, save }) {
  ...
  const r32auto = useMemo(() => {
    const list = [];
    GK.forEach(g => {
      if (standings[g]?.[0]) list.push(standings[g][0].team);
      if (standings[g]?.[1]) list.push(standings[g][1].team);
    });
    best8.forEach(t => list.push(t.team));
    return list;
  }, [standings, best8]);
    ...
    <div>
      <Card sx={{ marginBottom:10 }}>
        ...
        <div style={{ color:C.muted, fontSize:12, marginBottom:12 }}>
          Calculado a partir de los resultados reales ingresados. Guarda para activar el scoring.
        </div>
        <div style={{ marginBottom:10 }}>
          <div style={{ fontSize:11, color:C.muted, marginBottom:6 }}>
            1ros y 2dos ({GK.flatMap(g=>[standings[g]?.[0],standings[g]?.[1]]).filter(s=>s?.team).length}/24)
          </div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
            {GK.flatMap(g=>[standings[g]?.[0],standings[g]?.[1]]).filter(s=>s?.team).map(s => (
              <span key={s.team} style={{ background:'#1a3a10', color:C.grn, border:`1px solid ${C.grn}33`, borderRadius:5, padding:'3px 8px', fontSize:11 }}>{s.team}</span>
            ))}
          </div>
        </div>
        <div>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
            {best8.map(s => (
              <span key={s.team} style={{ background:'#1a280a', color:'#a3d977', border:`1px solid #a3d97733`, borderRadius:5, padding:'3px 8px', fontSize:11 }}>
                {s.team} <span style={{ opacity:.6 }}>{s.pts}p {s.gd>=0?'+':''}{s.gd}</span>
              </span>
            ))}
          </div>
        </div>
      </Card>
      <Btn label={ok?'✓ Guardado':'Guardar clasificados reales'} v="green"
        onClick={async () => { await save({ ...rr, cl:{ r32all:r32auto } }); setOk(true); }} />
    </div>
  );
}
...
function APl({ rr }) {
  ...
  useEffect(() => {
    S.list('p:').then(async keys => {
      const d = await Promise.all(keys.map(k => S.get(k)));
      setList(d.filter(Boolean).sort((a,b) => a.name.localeCompare(b.name)));
      setLoading(false);
    });
  }, []);
  ...
  const del = async p => {
    ...
    await S.del(pk(p.name));
    setList(l => l.filter(x => x.name !== p.name));
    setSelected(null);
  };
          ...
          return (
            <Card key={g} sx={{ marginBottom:8 }}>
              ...
              {matches.map(({ pair:[i,j] }) => {
                ...
                return (
                  <div key={k} style={{ display:'flex', alignItems:'center', padding:'5px 0', borderTop:`1px solid ${C.brd}`, fontSize:11 }}>
                    <span style={{ flex:1, textAlign:'right', paddingRight:6, color:C.muted }}>{teams[i]}</span>
                    <span style={{ minWidth:50, textAlign:'center', fontWeight:700, color:hasPred?C.txt:C.muted+'55' }}>
                      {hasPred ? `${pred.h}:${pred.a}` : '—'}
                    </span>
                    <span style={{ color:C.muted, fontSize:9, minWidth:30, textAlign:'center' }}>
                      {hasReal ? `real:${real.h}:${real.a}` : '?'}
                    </span>
                    <span style={{ flex:1, paddingLeft:6, color:C.muted }}>{teams[j]}</span>
                    <span style={{ minWidth:28, textAlign:'right', color:pts>0?(exact?C.gold:C.grn):C.muted, fontWeight:700, fontSize:12 }}>
                      {hasReal ? (pts>0?`+${pts}`:'0') : ''}
                    </span>
                  </div>
                );
        ...
        <Card sx={{ marginBottom:10 }}>
          ...
          <div style={{ display:'flex', flexWrap:'wrap', gap:4, marginTop:4 }}>
            {GK.flatMap(g=>[st[g]?.[0],st[g]?.[1]]).filter(s=>s?.team).map(s => (
              <span key={s.team} style={{ background:'#1a3a10', color:C.grn, border:`1px solid ${C.grn}33`, borderRadius:5, padding:'3px 7px', fontSize:11 }}>{s.team}</span>
            ))}
            {b8.map(s => (
              <span key={s.team} style={{ background:'#1a280a', color:'#a3d977', border:`1px solid #a3d97733`, borderRadius:5, padding:'3px 7px', fontSize:11 }}>{s.team}</span>
            ))}
          </div>
        </Card>
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
        <div style={{ color:C.muted, fontSize:13 }}>{list.length} registrados · {submitted.length} enviados</div>
        {submitted.length > 0 && (
          <Btn v="dark" full={false} sm label="📥 Exportar CSV" onClick={()=>exportToCSV(list, rr)} />
        )}
      </div>
      {submitted.length > 0 && (
        <div>
          ...
          {submitted.map(p => {
            const sc = calcTotal(p.t1, rr);
            return (
              <div key={p.name} onClick={()=>setSelected(p)} style={{
      ...
      {pending.length > 0 && (
        <div style={{ marginTop:14 }}>
          ...
          {pending.map(p => (
            <div key={p.name} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 0', borderBottom:`1px solid ${C.brd}` }}>
      ...
      {list.length === 0 && (
        <Card><div style={{ color:C.muted, textAlign:'center', padding:20 }}>Sin participantes registrados aún</div></Card>
      )}
    </div>
  );
}
...
function Participant({ cfg, back }) {
  ...
  if (!auth) return (
    <div style={{ maxWidth:380, margin:'0 auto', padding:'40px 20px' }}>
      <Back go={back} />
      <Title text="MIS PREDICCIONES" />
      <Card sx={{ marginTop:14 }}>
        <div style={{ display:'flex', gap:6, marginBottom:16 }}>
          {['Ingresar','Registrarse'].map((l,i) => (
            <button key={l} onClick={()=>setIsNew(!!i)} style={{
              flex:1, background:isNew===!!i?C.gold:C.card2,
              color:isNew===!!i?'#070f06':C.muted,
              border:`1px solid ${C.brd}`, borderRadius:7, padding:'9px',
              fontWeight:700, cursor:'pointer', fontFamily:"'Nunito',sans-serif", fontSize:13,
            }}>{l}</button>
          ))}
        </div>
  ...
  return (
    <div style={{ maxWidth:640, margin:'0 auto', padding:16 }}>
      ...
      {cfg?.phase==='setup' && (
        <Card><div style={{ color:C.muted, textAlign:'center', padding:'24px 0', fontSize:14 }}>⏳ La polla aún no está abierta.</div></Card>
      )}
      {open && !pd.t1?.submitted && <T1Form pd={pd} save={saveP} rrGm={rr?.gm||{}} />}
      {pd.t1?.submitted && <SubmittedView pd={pd} rr={rr} score={score} />}
      {!pd.t1?.submitted && !open && cfg?.phase !== 'setup' && (
        <Card><div style={{ color:C.muted, textAlign:'center', padding:20 }}>
          {cfg?.closingDate && new Date() > new Date(cfg.closingDate)
            ? '⏰ El tiempo de predicciones cerró.'
            : '⚠ Las predicciones no están disponibles en este momento.'}
        </div></Card>
      )}
    </div>
  );
  ...
  return (
    <div>
      ...
      {tab==='matches' && (
        <div>
          <div style={{ color:C.muted, fontSize:11, marginBottom:10 }}>
            Verde = resultado correcto · Dorado = marcador exacto
          </div>
          {GK.map(g => {
            const teams = GROUPS[g], matches = groupMatchList(g);
            ...
            return (
              <Card key={g} sx={{ marginBottom:8 }}>
                ...
                {matches.map(({ pair:[i,j] }) => {
                  const k = matchKey(g,i,j), pred = pd.t1?.gm?.[k], real = rr?.gm?.[k];
                  const { pts, exact } = scoreMatch(pred, real);
                  const hasPred = pred?.h!=null && pred?.a!=null && pred.h!=='' && pred.a!=='';
                  const hasReal = real?.h!=null && real?.a!=null && real.h!=='' && real.a!=='';
                  const correct = pts > 0;
                  return (
                    <div key={k} style={{ display:'flex', alignItems:'center', padding:'6px 0', borderTop:`1px solid ${C.brd}` }}>
      ...
      {tab==='r32' && (
        <div>
          <Card sx={{ marginBottom:10 }}>
            <Lbl text="1ros y 2dos de grupo (24 — automático)" />
          ...
          <div style={{ background:C.card2, border:`1px solid ${C.brd}`, borderRadius:10, padding:'12px 14px', fontSize:12, color:C.muted }}>
            ...
            Cuando la FIFA publique los cruces (aprox. 27 junio) se abrirá el Tiempo 2.
          </div>
        </div>
      )}
    </div>
...
function T1Form({ pd, save, rrGm }) {
  ...
  const r32teams = useMemo(() => {
    const list = [];
    GK.forEach(g => {
      if (allStandings[g]?.[0]?.team) list.push(allStandings[g][0].team);
      if (allStandings[g]?.[1]?.team) list.push(allStandings[g][1].team);
    });
    best8.forEach(t => list.push(t.team));
    return list;
  }, [allStandings, best8]);
  ...
  const submit = async () => {
    setSaving(true);
    await save({ ...pd, t1:{ gm, standings:allStandings, r32teams, submitted:true } });
    setSaving(false);
  };
  ...
  return (
    <div>
      ...
      {!isLast && <GroupForm g={GK[step]} gm={gm} setGM={setGM} standings={allStandings[GK[step]]} rrGm={rrGm} />}
...
function GroupForm({ g, gm, setGM, standings, rrGm }) {
  const teams = GROUPS[g], matches = groupMatchList(g);
  const filled = matches.filter(({ pair:[i,j] }) => {
    const v = gm[matchKey(g,i,j)];
    return v && v.h!=null && v.a!=null && v.h!=='' && v.a!=='';
  }).length;
  const played = matches.filter(({ pair:[i,j] }) => {
    const v = (rrGm||{})[matchKey(g,i,j)];
    return v && v.h!=null && v.a!=null && v.h!=='' && v.a!=='';
  }).length;
  ...
  return (
    <div>
      <Card sx={{ marginBottom:10 }}>
        ...
        {played > 0 && (
          <div style={{ background:'#1a1a0a', border:`1px solid ${C.gold}33`, borderRadius:7, padding:'8px 12px', marginBottom:12, fontSize:11, color:C.gold }}>
        ...
        {[1,2,3].map(j => (
          <div key={j}>
            ...
            {matches.filter(m=>m.jornada===j).map(({ pair:[i,k] }) => {
              ...
              return (
                <div key={mk} style={{ display:'flex', alignItems:'center', padding:'9px 0', borderBottom:`1px solid ${C.brd}`, opacity:isPlayed?.6:1 }}>
      ...
      <Card>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
          ...
          <tbody>
            {(standings||[]).map((s,i) => (
              <tr key={s.team} style={{ borderTop:`1px solid ${C.brd}` }}>
  ...
  return (
    <div>
      <Card sx={{ marginBottom:10 }}>
        ...
        {GK.map(g => (
          <div key={g} style={{ display:'flex', justifyContent:'space-between', padding:'4px 0', borderBottom:`1px solid ${C.brd}`, fontSize:12 }}>
      ...
      <Card sx={{ marginBottom:10 }}>
        ...
        <div style={{ fontSize:11, color:C.muted, marginBottom:8 }}>24 primeros/segundos + 8 mejores terceros — calculados automáticamente</div>
        <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
          {GK.flatMap(g=>[allStandings[g]?.[0],allStandings[g]?.[1]]).filter(s=>s?.team).map(s => (
            <span key={s.team} style={{ background:'#1a3a10', color:C.grn, border:`1px solid ${C.grn}33`, borderRadius:5, padding:'3px 7px', fontSize:11 }}>{s.team}</span>
          ))}
          {best8.map(s => (
            <span key={s.team} style={{ background:'#1a280a', color:'#a3d977', border:`1px solid #a3d97733`, borderRadius:5, padding:'3px 7px', fontSize:11 }}>
              {s.team} <span style={{ opacity:.6, fontSize:10 }}>{s.pts}p</span>
            </span>
          ))}
        </div>
      </Card>
      <div style={{ background:C.card2, border:`1px solid ${C.brd}`, borderRadius:10, padding:'12px 14px', marginBottom:14, fontSize:12, color:C.muted }}>
        <span style={{ color:C.gold, fontWeight:700 }}>¿Qué sigue? </span>
        Cuando la FIFA publique los cruces de dieciseisavos (aprox. 27 junio), se abrirá el Tiempo 2.
      </div>
      {!confirm
...
function Leaderboard({ cfg, back }) {
  ...
  const ranked = list
    .filter(p => p.t1?.submitted)
    .map(p => ({ ...p, ...calcTotal(p.t1, rr) }))
    .sort((a,b) => b.total!==a.total ? b.total-a.total : b.exactAll-a.exactAll)
    .slice(0, 20);
  ...
  return (
    <div style={{ maxWidth:640, margin:'0 auto', padding:16 }}>
      ...
      {ranked.length === 0 && (
        <Card><div style={{ color:C.muted, textAlign:'center', padding:'30px 0' }}>Nadie ha enviado predicciones aún.</div></Card>
      )}
      {ranked.map((p,i) => (
        <div key={p.name} style={{
          display:'flex', alignItems:'center', gap:12,
          background:i===0?'#0f2208':C.card,
          border:`1px solid ${i===0?C.gold:C.brd}`,
          borderRadius:10, padding:'12px 16px', marginBottom:7,
        }}>
          <div style={{ fontFamily:"'Bebas Neue',cursive", fontSize:28, color:i<3?C.gold:C.muted, width:32, textAlign:'center', lineHeight:1 }}>
            {medals[i]||i+1}
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontWeight:700, fontSize:14 }}>{p.name}</div>
...
export default function App() {
  ...
  if (loading) return (
    <div style={{ background:'#070f06', minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'Nunito',sans-serif", color:C.gold, fontSize:18 }}>
      Cargando...
    </div>
  );
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar-thumb { background: #1c3419; border-radius: 4px; }
      `}</style>
